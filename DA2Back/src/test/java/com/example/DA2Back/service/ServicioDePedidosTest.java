...
class ServicioDePedidosTest {
    ...
    @Test
    @DisplayName("Debe listar pedidos pendientes asignables")
    void testListarPendientesAsignables() {
        when(pedidosRepository.findByEstadoAndRepartidorIsNull(EstadoPedido.CREADO))
                .thenReturn(List.of(pedidoMock));

        List<PedidoResponseDTO> lista = servicioDePedidos.listarPendientesAsignables();

        assertNotNull(lista);
        assertEquals(1, lista.size());
        assertEquals(EstadoPedido.CREADO, lista.get(0).getEstado());
        verify(pedidosRepository).findByEstadoAndRepartidorIsNull(EstadoPedido.CREADO);
    }
