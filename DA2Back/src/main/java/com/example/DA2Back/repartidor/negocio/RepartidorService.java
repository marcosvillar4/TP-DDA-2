...
public class RepartidorService implements IRepartidorService {
    ...
    public RepartidorResponseDTO crear(RepartidorCreateDTO dto) {
        ...
        if (usuario.getEstado() != EstadoUsuario.VALIDADO) {
            throw new IllegalStateException("El usuario repartidor debe estar validado antes de crear su perfil operativo");
        }
        ...
        String patente = normalizarObligatorio(dto.getPatente(), "La patente es obligatoria");
        if (repartidorRepository.existsByPatenteIgnoreCase(patente)) {
    ...
    public List<UsuarioRepartidorDisponibleDTO> listarUsuariosRepartidoresDisponibles() {
        return usuarioRepository.findAll()
                ...
                .filter(usuario -> usuario.getEstado() == EstadoUsuario.VALIDADO)
                .filter(usuario -> !repartidorRepository.existsByUsuarioId(usuario.getId()))
                .map(RepartidorMapper::toUsuarioDisponibleDTO)
                .toList();
    }
    ...
    private void validarCamposCreacion(RepartidorCreateDTO dto) {
        ...
        normalizarObligatorio(dto.getPatente(), "La patente es obligatoria");
        normalizarObligatorio(dto.getZona(), "La zona es obligatoria");
    }
